...
qualsDataUnq <- qualsData2 %>%
  arrange(Time24Hour, Meeting, Horse, desc(V_Model_Avg))

qualsDataUnq <- distinct(qualsDataUnq, Time24Hour, Meeting, Horse, .keep_all = T)

head(qualsDataUnq)

mean(qualsDataUnq$BFSP_PL)
sum(qualsDataUnq$BFSP_PL)

dqUnique <- qualsDataUnq %>%
  filter(V_Model_Avg > 0, Model_Avg > 0)

write_csv(dqUnique, "Dual_Quals_Model_Avg_Nov18.csv")


dqDataNov <- read_csv("Dual_Quals_Model_Avg_Nov18.csv", col_names = T)


dqDataNov <- dqDataNov %>%
  mutate(Total_Avg = V_Model_Avg + Model_Avg)

dqDataNov <- dqDataNov %>%
  mutate(Total_Avg_Range = cut(Total_Avg, breaks = c(-1, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 10000),
                               labels = c("0.1", "0.5", "1.0", "2.0", "5.0", "10.0", ">10"),
                               ordered_result = T))

head(dqDataNov[,67:68])

dqDataNov %>%
  filter(Total_Avg >= 1.0) %>%
  group_by() %>%
  mutate(Won = if_else(BFSP_PL > 0, 1, 0)) %>%
  summarise(Runs = n(),
            Winners = sum(Won),
...
colnames(qualsDataUnq)

qualsDataUnq <- qualsDataUnq %>%
  mutate(V_Models_Avg_Range = cut(V_Model_Avg, breaks = c(-1000, 0.0, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 10000),
                               labels = c("<=0", "0.1", "0.5", "1.0", "2.0", "5.0", "10.0", ">10"),
                               ordered_result = T),
         Base_Models_Avg_Range = cut(Model_Avg, breaks = c(-1000, 0.0, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 10000),
                                     labels = c("<=0", "0.1", "0.5", "1.0", "2.0", "5.0", "10.0", ">10"),
                                     ordered_result = T))


head(qualsDataUnq[60:68], 20)
...
qualsDataUnq %>%
  group_by(Base_Models_Avg_Range) %>%
  mutate(Won = if_else(BFSP_PL > 0, 1, 0)) %>%
  summarise(Runs = n(),
            Winners = sum(Won),
            WinPercent = mean(Won),
